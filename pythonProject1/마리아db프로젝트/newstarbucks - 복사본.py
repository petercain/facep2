        ...
        if self.money - self.price >= 0:
            for m in gume:
                ...
                cur.execute("INSERT INTO order_data(ID, Menu, Price, Day, Time,Branch_name)VALUES(?,?,?,?,?,?)",
                            (Login.Id, m, pri,now.strftime("%Y.%m.%d"), now.strftime("%H:%M"), "광주신가점"))
    ...
    def btn_clicked_16(self):
        ...
        for m in gume:
            if m == "아메리카노":
                ...
                pri = 2900
            cur.execute("INSERT INTO order_data(ID, Menu, Price, Day, Time,Branch_name)VALUES(?,?,?,?,?,?)",
                        (Login.Id, m, pri, now.strftime("%Y.%m.%d"), now.strftime("%H:%M"), "영국본점"))
        QMessageBox.about(self, " ", "계산이 완료되었습니다.")
        self.kdialog.close()
        self.price = 0
        self.ame = 0
...
class pageThree(QMainWindow):
    ...
    def gume(self):
        ...
        conn = sqlite3.connect("Star_Bucks.db", isolation_level=None)
        cur = conn.cursor()
        cur.execute("SELECT * FROM order_data WHERE ID = '{}'".format(Login.Id))
        result = cur.fetchall()
...
class pageTwo(QMainWindow):
    ...
    def good(self):
        ...
        if len(self.textEdit_id2.toPlainText()) == 0:
            ...
            cur.execute("INSERT INTO join_data(ID, PASS, N_name)values(?,?,?)",(self.textEdit_id2.toPlainText(),self.textEdit_pass2r.toPlainText(),self.textEdit_nick.toPlainText()))
            self.pageOne = pageOne()
            self.pageOne.show()
            self.close()
